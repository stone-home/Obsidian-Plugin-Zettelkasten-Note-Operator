		...
		const scripts: Array<{
			...
			},
			{
				...
				script: join([
					...
					"  const debugLine = (label, value) => { const p = debugOut.createEl('div'); p.textContent = label + ': ' + (value ?? '(empty)'); };",
					...
					"  const refreshResult = debugOut.createEl('div', { cls: 'zk-pqa-debug-result' });",
					"  refreshBtn.onclick = async () => {",
					"    const key = keyForRefresh();",
					"    refreshResult.textContent = '';",
					"    refreshResult.style.color = '';",
					"    try {",
					"      await zk.refreshCodeProject(projectPath, key);",
					"      refreshResult.textContent = 'Last Refresh: OK';",
					"      refreshResult.style.color = 'var(--interactive-success)';",
					"    } catch (err) {",
					"      const msg = err instanceof Error ? err.message : String(err);",
					"      refreshResult.textContent = 'Refresh failed: ' + msg;",
					"      refreshResult.style.color = 'var(--text-error)';",
