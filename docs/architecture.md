...
| **service/projects/researchManager.ts** | Research project lifecycle: create project/dashboard, objectives, steps, experiments, requirements, drafts; update dashboard frontmatter; push materials to GitHub. | Via `window.ZettelkastenOperator` (e.g. `createResearchObjective`, `createResearchDraft`, `pushToGitHub`) or from main when registering global actions. Each call instantiates `new ResearchManager(app, settings)`. |
...
| **service/compiler/promptGenerator.ts** | Generate AI prompt from draft: extract/classify links, clean narrative, build prompt markdown, save to `project/prompts/`. | From main command `generate-ai-prompt` or via `window.ZettelkastenOperator.generateAIPrompt(draftFileOrPath)`. |
